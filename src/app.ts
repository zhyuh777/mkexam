...
async function loadExamConfig(): Promise<void> {
  ...
  // 先加载预设列表
  const _presets = await GET<Record<string, [string, string, number, number][]>>("/api/presets");
  ...
  sel.onchange = () => {
    ...
    if (ps && _presets) {
      ...
      let matched = '';
      for (const pn of Object.keys(_presets)) {
        if (cur.includes(pn)) { matched = pn; break; }
      }
      ps.value = matched;
    }
    onExamSubjectChange();
...
function setHeaderDefaults(name: string): void {
  ...
  const hAuthor = document.getElementById("h-author") as HTMLInputElement;
  const hReviewer = document.getElementById("h-reviewer") as HTMLInputElement;
  if (hCourse) hCourse.value = displayName;
  if (hYear) hYear.value = name.includes("大数据") ? "2025" : (name.includes("电工") ? "2025" : "2024");
  if (hDept) hDept.value = name.includes("大数据") ? "信息工程学院 大数据技术专业" : (name.includes("电工") ? "" : "信息工程学院 人工智能技术应用专业");
  if (hAuthor) hAuthor.value = name.includes("大数据") ? "曾薇" : "";
  if (hReviewer) hReviewer.value = name.includes("大数据") ? "王亮" : "";
}
...
async function onExamSubjectChange(): Promise<void> {
  ...
  if (ps && ps.value) {
    try {
      const presets = await GET<Record<string, [string, string, number, number][]>>("/api/presets");
      ...
      if (secs) {
        ...
        for (const [title, key, count, score] of secs) {
          ...
          const countInput = row.querySelector(".cfg-count") as HTMLInputElement;
          const scoreInput = row.querySelector(".cfg-score") as HTMLInputElement;
